...
router.post('/register', (req,res)=> {
    ...
    // question marks after values are the inputs to corresponding positions inside the ()'s before VALUES
    ...
    connection.query(insertIntoCust, [name, city, state, 1337, creditLimit], (error, results) => {
        ...
        connection.query(insertQuery, [newID, accountType, password, currTimeStamp, token], (error, results) => {
            if (error) {
                res.json({
                    msg: error
                })
